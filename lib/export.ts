...
export async function exportToPDF(title: string, content: string): Promise<void> {
  ...
  // Use an iframe to completely isolate from oklch CSS variables
  // html2canvas cannot parse oklch() — we must avoid any inheritance
...
<html><head><style>
  * { margin:0; padding:0; box-sizing:border-box; }
  ...
  .pdf-title {
    ...
    margin:0 0 8px; line-height:1.3; letter-spacing:-0.3px;
  }
  .pdf-date {
    font-size:11px; color:#888; margin:0 0 24px;
    font-family:ui-monospace,SFMono-Regular,monospace;
  }
  .pdf-hr {
    border:none; border-top:1px solid #e0e0e0; margin:0 0 24px;
  }
  ...
  <p class="pdf-date">${new Date().toLocaleDateString('en-US', { year: 'numeric', month: 'long', day: 'numeric' })}</p>
  <hr class="pdf-hr">
  <div>${html}</div>
</body></html>`)
  iframeDoc.close()

  // Wait for rendering (images, fonts, KaTeX)
